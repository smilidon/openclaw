...
final class OpenClawSnapshotUITests: XCTestCase {
    ...
    func testChatComposerStartsCompactAndGrowsWithDraft() throws {
        ...
        let inlineSelectedModel = app.buttons["openai/gpt-5.6-luna"]
        XCTAssertTrue(inlineSelectedModel.waitForExistence(timeout: 3))
        XCTAssertTrue(app.buttons["Default: openai/gpt-5.6-luna"].exists)
        ...
        XCTAssertTrue(app.buttons["openai/gpt-5.6-luna"].exists)
        app.buttons["openai/gpt-5.6-luna"].tap()
        ...
        self.waitForValue("gpt-5.6-luna", of: restoredInlineModel)
...
extension OpenClawSnapshotUITests {
    ...
    func testChatActionsModelRowsScreenshot() throws {
        ...
        let defaultModel = app.buttons["Default: openai/gpt-5.6-luna"]
        ...
        let explicitModel = app.buttons["openai/gpt-5.6-luna"]
        let initialExplicitModelCount = app.buttons.matching(
            NSPredicate(format: "label == %@", "openai/gpt-5.6-luna")).count
        providerDrawer.tap()
        ...
        XCTAssertGreaterThan(
            app.buttons.matching(NSPredicate(format: "label == %@", "openai/gpt-5.6-luna")).count,
        ...
        let restoredDefaultModel = app.buttons["Default: openai/gpt-5.6-luna"]
        XCTAssertTrue(restoredDefaultModel.waitForExistence(timeout: 5))
        restoredDefaultModel.tap()
        XCTAssertTrue(popover.waitForNonExistence(timeout: 3))

        actions.tap()
        let reselectedDefaultModel = app.buttons["Default: openai/gpt-5.6-luna"]
        ...
        let settledDefaultModel = app.buttons["Default: openai/gpt-5.6-luna"]
        ...
        reopenedActions.tap()
        let reopenedDefaultModel = app.buttons["Default: openai/gpt-5.6-luna"]
        XCTAssertTrue(reopenedDefaultModel.waitForExistence(timeout: 5))
        XCTAssertEqual(reopenedDefaultModel.label, "Default: openai/gpt-5.6-luna")
